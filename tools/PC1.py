...
save_param_dir = '../result/inferred_parameter/PC1'

# function
def recordPC1Info(hyper_list, model_file, csv_file):
...
class PC1:
    def __init__(self, groundtruth_DataFrame, review_DataFrame):
        ...
        self.stanData = {
            ...
            'sender':self.sender, 'receiver':self.receiver, 'corrected':self.corrected,
            'senderOrigin':self.sender_origin_id, 'hyper':[1]*4
            }
        self.stanmodel = pystan.StanModel(file=PC1_path)
    ...
    def corrcoefWithTruth(self,hyper_list):
        ...
        if RECORD == True:
            ...
            # save
            pkl_name = 'PC1-{}.pkl'.format(timeStamp())
            saveStanExtract(parameters, os.path.join(save_param_dir,pkl_name))
            # record table
            recordPC1Info(hyper_list,pkl_name,os.path.join(save_param_dir,'models.csv'))
        # calculate corrcoef
