...
# path information
PG3PC1PC2_path = '../models/PG3PC1PC2.stan'
save_param_dir = '../result/inferred_parameter/PG3PC1PC2'

# function
def recordPG3PC1PC2Info(hyper_list, model_file, csv_file):
    if os.path.exists(csv_file):
        df = pd.read_csv(csv_file)
    else:
        df = pd.DataFrame(columns=['file','mu0','gamma0','theta0','theta1','eta0','kappa0','kappa1'])
...
class PG3PC1PC2:
    def __init__(self, groundtruth_DataFrame, review_DataFrame):
        ...
        corrected = self.rDF['corrected'].get_values()
        self.corrected = (~corrected.astype(np.bool)).astype(np.int64)
        ...
        self.stanData = {
            ...
            'corrected':self.corrected, 'diff':self.diff, 'senderOrigin':self.sender_origin_id, 'hyper':[1]*7
            }
        self.stanmodel = pystan.StanModel(file=PG3PC1PC2_path)
    ...
    def fit(self, hyper_list, iteration=5000, chains=4, warmup=500, n_jobs=-1, algorithm='NUTS'):
        ...
        '''
        if len(hyper_list) != 7:
            print('NError: {} given. Set 7 hyper parameters.'.format(len(hyper_list)))
            sys.exit(1)
    ...
    def corrcoefWithTruth(self,hyper_list):
        ...
        if RECORD == True:
            ...
            inferred_noise0 = ext['noise0'].mean(axis=0)
            inferred_noise1 = ext['noise1'].mean(axis=0)
            parameters = {'ability':inferred_ability, 'reliability':inferred_reliability,
                    'bias':inferred_bias, 'noise0':inferred_noise0, 'noise1':inferred_noise1}
            # save
            pkl_name = 'PG3PC1PC2-{}.pkl'.format(timeStamp())
            saveStanExtract(parameters, os.path.join(save_param_dir,pkl_name))
            # record table
            recordPG3PC1PC2Info(hyper_list,pkl_name,os.path.join(save_param_dir,'models.csv'))
        # calculate corrcoef
