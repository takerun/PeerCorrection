...
save_param_dir = '../result/inferred_parameter/PCG3'

# function
def recordPCG3Info(hyper_list, model_file, csv_file):
...
class PCG3:
    def __init__(self, groundtruth_DataFrame, review_DataFrame):
        ...
        self.stanmodel = pystan.StanModel(file=PCG3_path)
    ...
    def corrcoefWithTruth(self,hyper_list):
        ...
        if RECORD == True:
            ...
            # save
            pkl_name = 'PCG3-{}.pkl'.format(timeStamp())
            saveStanExtract(parameters, os.path.join(save_param_dir,pkl_name))
            # record table
            recordPCG3Info(hyper_list,pkl_name,os.path.join(save_param_dir,'models.csv'))
        # calculate corrcoef
