...
for i in xrange(trial_num):
    ...
    # model setting
    pcg1gm = PCG1GM(trainDF, valDF, testDF)
    ...
    myBopt = GPyOpt.methods.BayesianOptimization(f=pcg1gm.FunctionforGPyOpt,domain=bounds,
            initial_design_numdata=tune_trial_num,acquisition_type='LCB')
    saveGPyOpt(myBopt,i)

    # testing
    optX = myBopt.x_opt
    rmse = pcg1gm.rmseForTest(optX)
    #auc = pcg1gm.aucForTest(optX)
    with open(os.path.join(snapshot_path, 'result.txt'),'a') as f:
        f.write('##PCG1GM,split{}##\n'.format(i))
        f.write('x_opt: {}\n'.format(myBopt.x_opt))
