...
# path information
PCG1_path = '../models/PCG1.stan'
save_param_dir = '../result/inferred_parameter/PCG1'

# function
def recordPCG1Info(hyper_list, model_file, csv_file):
    if os.path.exists(csv_file):
        df = pd.read_csv(csv_file)
    else:
        df = pd.DataFrame(columns=['file','mu0','gamma0','alpha0','beta0','eta0','kappa0'])
...
class PCG1:
    def __init__(self, groundtruth_DataFrame, review_DataFrame):
        ...
        self.stanmodel = pystan.StanModel(file=PCG1_path)
    ...
    def corrcoefWithTruth(self,hyper_list):
        ...
        if RECORD == True:
            ...
            # save
            pkl_name = 'PCG1-{}.pkl'.format(timeStamp())
            saveStanExtract(parameters, os.path.join(save_param_dir,pkl_name))
            # record table
            recordPCG1Info(hyper_list,pkl_name,os.path.join(save_param_dir,'models.csv'))
        # calculate corrcoef
