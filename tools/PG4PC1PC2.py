...
# path information
PG4PC1PC2_path = '../models/PG4PC1PC2.stan'
save_param_dir = '../result/inferred_parameter/PG4PC1PC2'

# function
def recordPG4PC1PC2Info(hyper_list, model_file, csv_file):
    if os.path.exists(csv_file):
        df = pd.read_csv(csv_file)
    else:
        df = pd.DataFrame(columns=['file','mu0','gamma0','beta0','eta0','kappa0','kappa1'])
...
class PG4PC1PC2:
    def __init__(self, groundtruth_DataFrame, review_DataFrame):
        ...
        self.stanData = {
            ...
            'corrected':self.corrected, 'diff':self.diff, 'senderOrigin':self.sender_origin_id, 'hyper':[1]*6
            }
        self.stanmodel = pystan.StanModel(file=PG4PC1PC2_path)
    ...
    def corrcoefWithTruth(self,hyper_list):
        ...
        if RECORD == True:
            ...
            # save
            pkl_name = 'PG4PC1PC2-{}.pkl'.format(timeStamp())
            saveStanExtract(parameters, os.path.join(save_param_dir,pkl_name))
            # record table
            recordPG4PC1PC2Info(hyper_list,pkl_name,os.path.join(save_param_dir,'models.csv'))
        # calculate corrcoef
