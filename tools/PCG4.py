...
# path information
PCG4_path = '../models/PCG4.stan'
save_param_dir = '../result/inferred_parameter/PCG4'

# function
def recordPCG4Info(hyper_list, model_file, csv_file):
    if os.path.exists(csv_file):
        df = pd.read_csv(csv_file)
    else:
        df = pd.DataFrame(columns=['file','mu0','gamma0','beta0','eta0','kappa0'])
...
class PCG4:
    def __init__(self, groundtruth_DataFrame, review_DataFrame):
        ...
        self.stanData = {
            ...
            'corrected':self.corrected, 'senderOrigin':self.sender_origin_id, 'hyper':[1]*5
            }
        self.stanmodel = pystan.StanModel(file=PCG4_path)
    ...
    def corrcoefWithTruth(self,hyper_list):
        ...
        if RECORD == True:
            ...
            # save
            pkl_name = 'PCG4-{}.pkl'.format(timeStamp())
            saveStanExtract(parameters, os.path.join(save_param_dir,pkl_name))
            # record table
            recordPCG4Info(hyper_list,pkl_name,os.path.join(save_param_dir,'models.csv'))
        # calculate corrcoef
