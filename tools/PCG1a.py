...
# path information
PCG1a_path = '../models/PCG1a.stan'
save_param_dir = '../result/inferred_parameter/PCG1a'

# function
def recordPCG1aInfo(hyper_list, model_file, csv_file):
    if os.path.exists(csv_file):
        df = pd.read_csv(csv_file)
    else:
        df = pd.DataFrame(columns=['file','mu0','gamma0','alpha0','beta0','eta0','kappa0','scale0'])
...
class PCG1a:
    def __init__(self, groundtruth_DataFrame, review_DataFrame):
        ...
        self.stanmodel = pystan.StanModel(file=PCG1a_path)
    ...
    def corrcoefWithTruth(self,hyper_list):
        ...
        if RECORD == True:
            ...
            # save
            pkl_name = 'PCG1a-{}.pkl'.format(timeStamp())
            saveStanExtract(parameters, os.path.join(save_param_dir,pkl_name))
            # record table
            recordPCG1aInfo(hyper_list,pkl_name,os.path.join(save_param_dir,'models.csv'))
        # calculate corrcoef
