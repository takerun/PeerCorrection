...
for i in xrange(trial_num):
    ...
    # model setting
    pg5gm = PG5GM(trainDF, valDF, testDF)
    ...
    myBopt = GPyOpt.methods.BayesianOptimization(f=pg5gm.FunctionforGPyOpt,domain=bounds,
            initial_design_numdata=tune_trial_num,acquisition_type='LCB')
    saveGPyOpt(myBopt,i)

    # testing
    optX = myBopt.x_opt
    rmse = pg5gm.rmseForTest(optX)
