...
for i in xrange(trial_num):
    ...
    # model setting
    pg1gm = PG1GM(trainDF, valDF, testDF)
    ...
    myBopt = GPyOpt.methods.BayesianOptimization(f=pg1gm.FunctionforGPyOpt,domain=bounds,
            initial_design_numdata=tune_trial_num,acquisition_type='LCB')
    saveGPyOpt(myBopt,i)

    # testing
    optX = myBopt.x_opt
    rmse = pg1gm.rmseForTest(optX)
