...
for i in xrange(trial_num):
    ...
    # model setting
    pg3gm = PG3GM(trainDF, valDF, testDF)
    ...
    myBopt = GPyOpt.methods.BayesianOptimization(f=pg3gm.FunctionforGPyOpt,domain=bounds,
            initial_design_numdata=tune_trial_num,acquisition_type='LCB')
    saveGPyOpt(myBopt,i)

    # testing
    optX = myBopt.x_opt
    rmse = pg3gm.rmseForTest(optX)
