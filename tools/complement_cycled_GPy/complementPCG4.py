...
from PCG4GM import PCG4GM

# path information
trial_num = 5
tune_trial_num = 100
snapshot_path = '../result/snapshot/complement'
snapshot_form = 'PCG4-hyper5-EI5000w500-split{0}-{1}'
...
for i in xrange(trial_num):
    ...
    # model setting
    pcg4gm = PCG4GM(trainDF, valDF, testDF)
    ...
    myBopt = GPyOpt.methods.BayesianOptimization(f=pcg4gm.FunctionforGPyOpt,domain=bounds,initial_design_numdata=tune_trial_num,acquisition_type='LCB')
    saveGPyOpt(myBopt,i)

    # testing
    optX = myBopt.x_opt
    rmse = pcg4gm.rmseForTest(optX)
    #auc = pcg4gm.aucForTest(optX)
    with open(os.path.join(snapshot_path, 'result.txt'),'a') as f:
        f.write('##PCG4GM,split{}##\n'.format(i))
        f.write('x_opt: {}\n'.format(myBopt.x_opt))
