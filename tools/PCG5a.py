...
# path information
PCG5a_path = '../models/PCG5a.stan'
save_param_dir = '../result/inferred_parameter/PCG5a'

# function
def recordPCG5aInfo(hyper_list, model_file, csv_file):
    if os.path.exists(csv_file):
        df = pd.read_csv(csv_file)
    else:
        df = pd.DataFrame(columns=['file','mu0','gamma0','beta0','eta0','kappa0','lambda0','scale0'])
...
class PCG5a:
    def __init__(self, groundtruth_DataFrame, review_DataFrame):
        ...
        self.stanData = {
            ...
            'corrected':self.corrected, 'senderOrigin':self.sender_origin_id, 'hyper':[1]*7
            }
        self.stanmodel = pystan.StanModel(file=PCG5a_path)
    ...
    def corrcoefWithTruth(self,hyper_list):
        ...
        if RECORD == True:
            ...
            # save
            pkl_name = 'PCG5a-{}.pkl'.format(timeStamp())
            saveStanExtract(parameters, os.path.join(save_param_dir,pkl_name))
            # record table
            recordPCG5aInfo(hyper_list,pkl_name,os.path.join(save_param_dir,'models.csv'))
        # calculate corrcoef
