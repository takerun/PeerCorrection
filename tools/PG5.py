...
# path information
PG5_path = '../models/PG5.stan'
save_param_dir = '../result/inferred_parameter/PG5'

# function
def recordPG5Info(hyper_list, model_file, csv_file):
    if os.path.exists(csv_file):
        df = pd.read_csv(csv_file)
    else:
        df = pd.DataFrame(columns=['file','mu0','gamma0','beta0','eta0','lambda0'])
...
class PG5:
    def __init__(self, groundtruth_DataFrame, review_DataFrame):
        ...
        self.stanmodel = pystan.StanModel(file=PG5_path)
    ...
    def corrcoefWithTruth(self,hyper_list):
        ...
        if RECORD == True:
            ...
            # save
            pkl_name = 'PG5-{}.pkl'.format(timeStamp())
            saveStanExtract(parameters, os.path.join(save_param_dir,pkl_name))
            # record table
            recordPG5Info(hyper_list,pkl_name,os.path.join(save_param_dir,'models.csv'))
        # calculate corrcoef
