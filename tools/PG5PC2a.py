...
# path information
PG5PC2a_path = '../models/PG5PC2a.stan'
save_param_dir = '../result/inferred_parameter/PG5PC2a'

# function
def recordPG5PC2aInfo(hyper_list, model_file, csv_file):
...
class PG5PC2a:
    def __init__(self, groundtruth_DataFrame, review_DataFrame):
        ...
        self.stanData = {
            ...
            'diff':self.diff, 'senderOrigin':self.sender_origin_id, 'hyper':[1]*7
            }
        self.stanmodel = pystan.StanModel(file=PG5PC2a_path)
    ...
    def corrcoefWithTruth(self,hyper_list):
        ...
        if RECORD == True:
            ...
            # save
            pkl_name = 'PG5PC2a-{}.pkl'.format(timeStamp())
            saveStanExtract(parameters, os.path.join(save_param_dir,pkl_name))
            # record table
            recordPG5PC2aInfo(hyper_list,pkl_name,os.path.join(save_param_dir,'models.csv'))
        # calculate corrcoef
