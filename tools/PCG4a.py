...
# path information
PCG4a_path = '../models/PCG4a.stan'
save_param_dir = '../result/inferred_parameter/PCG4a'

# function
def recordPCG4aInfo(hyper_list, model_file, csv_file):
    if os.path.exists(csv_file):
        df = pd.read_csv(csv_file)
    else:
        df = pd.DataFrame(columns=['file','mu0','gamma0','beta0','eta0','kappa0','scale0'])
...
class PCG4a:
    def __init__(self, groundtruth_DataFrame, review_DataFrame):
        ...
        self.stanData = {
            ...
            'corrected':self.corrected, 'senderOrigin':self.sender_origin_id, 'hyper':[1]*6
            }
        self.stanmodel = pystan.StanModel(file=PCG4a_path)
    ...
    def corrcoefWithTruth(self,hyper_list):
        ...
        if RECORD == True:
            ...
            # save
            pkl_name = 'PCG4a-{}.pkl'.format(timeStamp())
            saveStanExtract(parameters, os.path.join(save_param_dir,pkl_name))
            # record table
            recordPCG4aInfo(hyper_list,pkl_name,os.path.join(save_param_dir,'models.csv'))
        # calculate corrcoef
