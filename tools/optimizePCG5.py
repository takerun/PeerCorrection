...
snapshot_form = 'PCG5-hyper6-EI5000w500-fold{0}-{1}'
...
for i in xrange(train_fold_num):
    ...
    # testing
    optX = myBopt.x_opt
    testModel = PCG5(gDF,testDF)
    cor = testModel.corrcoefWithTruth(optX)
    with open(os.path.join(snapshot_path, 'resultPCG5.txt'),'a') as f:
        f.write('##PCG5,split{}##\n'.format(i))
        f.write('x_opt: {}\n'.format(myBopt.x_opt))
