...
# path information
PG4PC2_path = '../models/PG4PC2.stan'
save_param_dir = '../result/inferred_parameter/PG4PC2'

# function
def recordPG4PC2Info(hyper_list, model_file, csv_file):
...
class PG4PC2:
    def __init__(self, groundtruth_DataFrame, review_DataFrame):
        ...
        self.stanData = {
            ...
            'diff':self.diff, 'senderOrigin':self.sender_origin_id, 'hyper':[1]*5
            }
        self.stanmodel = pystan.StanModel(file=PG4PC2_path)
    ...
    def corrcoefWithTruth(self,hyper_list):
        ...
        if RECORD == True:
            ...
            # save
            pkl_name = 'PG4PC2-{}.pkl'.format(timeStamp())
            saveStanExtract(parameters, os.path.join(save_param_dir,pkl_name))
            # record table
            recordPG4PC2Info(hyper_list,pkl_name,os.path.join(save_param_dir,'models.csv'))
        # calculate corrcoef
