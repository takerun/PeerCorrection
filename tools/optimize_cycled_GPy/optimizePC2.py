...
snapshot_form = 'PC2-hyper4-EI5000w500-fold{0}-{1}'
...
for i in xrange(train_fold_num):
    ...
    # testing
    optX = myBopt.x_opt
    testModel = PC2(gDF,testDF)
    cor = testModel.corrcoefWithTruth(optX)
    with open(os.path.join(snapshot_path, 'resultPC2.txt'),'a') as f:
        f.write('##PC2,split{}##\n'.format(i))
        f.write('x_opt: {}\n'.format(myBopt.x_opt))
