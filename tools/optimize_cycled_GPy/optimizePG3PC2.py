...
snapshot_form = 'PG3PC2-hyper6-EI5000w500-fold{0}-{1}'
...
for i in xrange(train_fold_num):
    ...
    # model setting
    trainModel = PG3PC2(gDF, trainDF)

    # training
    bounds = [
        {'name': 'mu0', 'type': 'continuous', 'domain': (0,4)},
        {'name': 'gamma0', 'type': 'continuous', 'domain': (1.0e-2,1.0e2)},
        {'name': 'theta0', 'type': 'continuous', 'domain': (1.0e-2,1.0e1)},
        {'name': 'theta1', 'type': 'continuous', 'domain': (1.0e-1,1.0e1)},
    ...
    # testing
    optX = myBopt.x_opt
    testModel = PG3PC2(gDF,testDF)
    cor = testModel.corrcoefWithTruth(optX)
    with open(os.path.join(snapshot_path, 'resultPG3PC2.txt'),'a') as f:
        f.write('##PG3PC2,split{}##\n'.format(i))
        f.write('x_opt: {}\n'.format(myBopt.x_opt))
