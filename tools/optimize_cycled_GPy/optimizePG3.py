...
snapshot_form = 'PG3-hyper5-EI5000w500-fold{0}-{1}'
...
for i in xrange(train_fold_num):
    ...
    # testing
    optX = myBopt.x_opt
    testModel = PG3(gDF,testDF)
    cor = testModel.corrcoefWithTruth(optX)
    with open(os.path.join(snapshot_path, 'resultPG3.txt'),'a') as f:
        f.write('##PG3,split{}##\n'.format(i))
        f.write('x_opt: {}\n'.format(myBopt.x_opt))
