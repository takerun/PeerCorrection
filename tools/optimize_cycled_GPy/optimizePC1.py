...
snapshot_form = 'PC1-hyper4-EI5000w500-fold{0}-{1}'
...
for i in xrange(train_fold_num):
    ...
    # testing
    optX = myBopt.x_opt
    testModel = PC1(gDF,testDF)
    cor = testModel.corrcoefWithTruth(optX)
    with open(os.path.join(snapshot_path, 'resultPC1.txt'),'a') as f:
        f.write('##PC1,split{}##\n'.format(i))
        f.write('x_opt: {}\n'.format(myBopt.x_opt))
